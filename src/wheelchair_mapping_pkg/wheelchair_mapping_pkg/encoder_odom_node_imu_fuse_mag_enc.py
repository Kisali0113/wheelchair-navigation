...
class EncoderOdomNode(Node):
    """Odometry using encoder + IMU  based velocity + steering (Ackermann model)"""
    ...
    def __init__(self):
        ...
        self.declare_parameter('velocity_fusion_alpha', 0.7)
        ...
        self.velocity_fusion_alpha = self.get_parameter('velocity_fusion_alpha').value
        ...
        self.velocity = 0.0  # Encoder velocity
        self.steer_angle = 0.0
        self.accel_x = 0.0  # IMU linear acceleration in body frame
        self.accel_y = 0.0
        self.imu_vel_x = 0.0  # IMU-integrated velocity
        self.imu_vel_y = 0.0
        self.fused_velocity = 0.0  # Fused estimate
    ...
    def imu_callback(self, msg: Imu):
        ...
        self.get_logger().info(f'Received IMU accel: ax={self.accel_x:.3f}, ay={self.accel_y:.3f}')
    ...
    def update_odometry(self):
        ...
        # Integrate IMU acceleration to get IMU velocity in body frame
        self.imu_vel_x += self.accel_x * dt
        self.imu_vel_y += self.accel_y * dt

        # Calculate IMU-based linear velocity magnitude
        imu_vel_mag = math.sqrt(self.imu_vel_x**2 + self.imu_vel_y**2)

        # Fuse encoder and IMU velocities (alpha blending)
        self.fused_velocity = self.velocity_fusion_alpha * self.velocity + (1.0 - self.velocity_fusion_alpha) * imu_vel_mag

        self.get_logger().info(
            f'Encoder vel: {self.velocity:.3f}, IMU vel: {imu_vel_mag:.3f}, Fused vel: {self.fused_velocity:.3f}')

        # Distance traveled using fused velocity
        delta_distance = self.fused_velocity * dt

        # Use magnetometer heading only for yaw
        self.yaw = self.heading

        # Update pose using current yaw and fused distance
        self.x += delta_distance * math.cos(self.yaw)
        self.y += delta_distance * math.sin(self.yaw)

        # Velocities for odometry message
        linear_velocity = self.fused_velocity
